...
def generate_graphs():
    ...
    try:
        ...
        output_path = os.path.join(temp_dir, "IR_graph_agg.png")
        plt.savefig(output_path)
        plt.close(fig)
        if os.path.exists(output_path):
            print(f"Saved IR graph to {output_path}")
        else:
            print(f"Failed to save IR graph: File not found at {output_path}")
    ...
    try:
        ...
        output_path = os.path.join(temp_dir, "NMR_H1_graph_agg.png")
        plt.savefig(output_path)
        plt.close(fig)
        if os.path.exists(output_path):
            print(f"Saved H1 NMR graph to {output_path}")
        else:
            print(f"Failed to save H1 NMR graph: File not found at {output_path}")
    except Exception as e:
    ...
    try:
        ...
        output_path = os.path.join(temp_dir, "NMR_C13_graph_agg.png")
        plt.savefig(output_path)
        plt.close(fig)
        if os.path.exists(output_path):
            print(f"Saved C13 NMR graph to {output_path}")
        else:
            print(f"Failed to save C13 NMR graph: File not found at {output_path}")
    except Exception as e:
    ...
    try:
        ...
        output_path = os.path.join(temp_dir, "UV_Vis_graph_agg.png")
        plt.savefig(output_path)
        plt.close(fig)
        if os.path.exists(output_path):
            print(f"Saved UV-Vis graph to {output_path}")
        else:
            print(f"Failed to save UV-Vis graph: File not found at {output_path}")
    except Exception as e:
