...
"""
ChemGrid Pro: Molecular Orbital Visualization with ORCA Cube Data
...
try:
    from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QPushButton,
                                 ...
                                 QTableWidget, QTableWidgetItem, QWidget, QSlider)
...
# Local imports
try:
    from cube_parser import parse_cube_file, extract_isosurface, get_slice_data, CubeData, BOHR_TO_ANGSTROM
    CUBE_AVAILABLE = True
except ImportError:
    CUBE_AVAILABLE = False
    logger.info("cube_parser not available")
...
class OrbitalParser:
    ...
    def parse_all_orbitals(filepath: Path) -> list:
        ...
        try:
            ...
            logger.error("OrbitalParser error: %s", e)
        return orbitals
            ...
            for i, line in enumerate(lines):
                if 'DIPOLE MOMENT' in line.upper():
                    for j in range(i+1, min(i+10, len(lines))):
                        ...
                        if 'Magnitude' in lines[j] or ('Total' in lines[j] and j > i+3):
                            parts = lines[j].split()
                            nums = [float(p) for p in parts if _is_float(p)]
                            if nums:
...
class OrbitalPlottingWidget(FigureCanvas):
    ...
    def plot_orbital_isosurface(self, cube, isovalue: float = 0.02,
                                 title: str = "Molecular Orbital"):
        """Plot orbital isosurface from cube data (3D gradient mesh + 2D contour).
        [FIX-3D-007] gradient isosurface: 각 face의 orbital amplitude에 따른
        colormap 그라데이션 적용 (HOMO=blue gradient, LUMO=red gradient).
        """
        self.figure.clear()
        ...
        if cube is None:
            ax3d.text2D(0.5, 0.5, 'No cube data\n(Run ORCA DFT first,\nthen click "Generate Cube Files")',
                       transform=ax3d.transAxes, ha='center', va='center', fontsize=11)
        ...
        # [FIX-3D-007v2] Chemically standard orbital phase coloring:
        #   positive lobe (+) = blue gradient, negative lobe (-) = red gradient
        #   This matches textbook convention (Atkins, Clayden, GaussView).
        #   Orbital identity (HOMO/LUMO) is conveyed by the title, not color.
        import matplotlib.colors as mcolors
        pos_cmap = plt.cm.Blues   # positive phase (+) for ALL orbitals
        neg_cmap = plt.cm.Reds    # negative phase (-) for ALL orbitals

        # 3D isosurface — [FIX-3D-007] gradient isosurface 적용
        try:
            ...
            if len(verts_pos) > 0:
                if len(faces_pos) > 0:
                    # [FIX-3D-007] face별 vertex 평균 거리 기반 gradient color
                    face_colors_pos = self._compute_gradient_colors(
                        verts_pos, faces_pos, cube, pos_cmap, alpha=0.70)
                    mesh_pos = Poly3DCollection(verts_pos[faces_pos], alpha=0.70,
                                                edgecolor='none', linewidth=0)
                    mesh_pos.set_facecolor(face_colors_pos)
                    ax3d.add_collection3d(mesh_pos)
                else:
                    # scatter fallback with gradient
                    dists = np.sqrt(np.sum(verts_pos**2, axis=1))
                    ax3d.scatter(verts_pos[:, 0], verts_pos[:, 1], verts_pos[:, 2],
                                c=dists, cmap=pos_cmap, s=8, alpha=0.6, edgecolors='none')

            if len(verts_neg) > 0:
                if len(faces_neg) > 0:
                    face_colors_neg = self._compute_gradient_colors(
                        verts_neg, faces_neg, cube, neg_cmap, alpha=0.70)
                    mesh_neg = Poly3DCollection(verts_neg[faces_neg], alpha=0.70,
                                                edgecolor='none', linewidth=0)
                    mesh_neg.set_facecolor(face_colors_neg)
                    ax3d.add_collection3d(mesh_neg)
                else:
                    dists = np.sqrt(np.sum(verts_neg**2, axis=1))
                    ax3d.scatter(verts_neg[:, 0], verts_neg[:, 1], verts_neg[:, 2],
                                c=dists, cmap=neg_cmap, s=8, alpha=0.6, edgecolors='none')
            ...
            ax3d.set_ylabel('Y (A)')
            ax3d.set_zlabel('Z (A)')
            ax3d.set_title(f'{title} (iso={isovalue:.3f})', fontsize=11)

        except Exception as e:
            logger.error("3D isosurface error: %s", e)
            ax3d.text2D(0.5, 0.5, f'3D Error: {e}', transform=ax3d.transAxes, ha='center')
        ...
        try:
            ...
            self.figure.colorbar(contour, ax=ax2d, label='Orbital amplitude')
            ...
            logger.error("2D contour error: %s", e)
            ax2d.text(0.5, 0.5, f'2D Error: {e}', transform=ax2d.transAxes, ha='center')

        self.figure.tight_layout()
        self.draw()

    def _compute_gradient_colors(self, verts, faces, cube, cmap, alpha=0.70):
        """[FIX-3D-007v2] face별 orbital amplitude 기반 gradient 색상 계산.

        Strategy (in priority order):
        1. Interpolate actual orbital amplitude at face centroids from cube grid data.
           This produces physically meaningful gradients where color intensity tracks
           the wavefunction magnitude on the isosurface.
        2. Fallback to molecular-center distance if interpolation fails.

        Args:
            verts: (N, 3) vertex coordinates (Angstrom)
        ...
        Returns:
            (M, 4) RGBA color array for each face
        """
        face_verts = verts[faces]  # (M, 3, 3)
        centroids = face_verts.mean(axis=1)  # (M, 3)

        # Try amplitude-based gradient from cube volumetric data
        values = self._interpolate_amplitude_at_points(centroids, cube)

        if values is not None and len(values) == len(centroids):
            # Use |amplitude| for gradient intensity
            abs_vals = np.abs(values)
        else:
            # Fallback: distance from molecular center
            if cube.atom_positions.shape[0] > 0:
                mol_center = (cube.atom_positions * BOHR_TO_ANGSTROM).mean(axis=0)
            else:
                mol_center = verts.mean(axis=0)
            abs_vals = np.sqrt(np.sum((centroids - mol_center)**2, axis=1))

        # 0~1 normalization
        v_min, v_max = abs_vals.min(), abs_vals.max()
        if v_max - v_min < 1e-10:
            normalized = np.full(len(abs_vals), 0.5)
        else:
            normalized = (abs_vals - v_min) / (v_max - v_min)

        # Colormap mapping (0.3~0.9 range to avoid overly bright/dark faces)
        mapped = 0.3 + normalized * 0.6
        colors = cmap(mapped)
        colors[:, 3] = alpha
    ...
    @staticmethod
    def _interpolate_amplitude_at_points(points_angstrom, cube):
        """Trilinear interpolation of cube volumetric data at given points.

        Args:
            points_angstrom: (M, 3) coordinates in Angstrom
            cube: CubeData with origin/step_vectors/data in Bohr

        Returns:
            (M,) array of interpolated values, or None on failure
        """
        try:
            from scipy.ndimage import map_coordinates
        except ImportError:
            return None  # scipy not available, use fallback

        try:
            origin = cube.origin * BOHR_TO_ANGSTROM
            step_sizes = cube.step_sizes_angstrom

            # Convert Angstrom coordinates to fractional grid indices
            if np.any(step_sizes < 1e-12):
                return None
            grid_coords = (points_angstrom - origin) / step_sizes  # (M, 3)

            # map_coordinates expects (3, M) shape
            coords_t = grid_coords.T  # (3, M)

            # Clamp to grid bounds to avoid extrapolation artifacts
            for i in range(3):
                coords_t[i] = np.clip(coords_t[i], 0, cube.data.shape[i] - 1)

            values = map_coordinates(cube.data, coords_t, order=1, mode='nearest')
            return values
        except Exception:
            return None

    def plot_electron_density(self, cube=None):
        ...
        if cube is not None:
            try:
                ...
                self.figure.colorbar(contour, ax=ax, label='Electron Density (a.u.)')
            ...
            self.figure.colorbar(contour, ax=ax, label='Electron Density (synthetic)')
            ax.set_xlabel('x')
            ax.set_ylabel('y')
    ...
    def plot_dipole_moment(self, dipole: DipoleMoment):
        ...
        ax.set_ylabel('y')
        ax.set_zlabel('z')
        ax.set_title(f'Dipole Moment (|mu| = {dipole.magnitude:.2f} D)', fontsize=13, fontweight='bold')
        self.figure.tight_layout()
        self.draw()

    def plot_mep(self, cube=None):
        ...
        if cube is not None:
            try:
                ...
                contour = ax.contourf(X, Y, vals, levels=20, cmap='RdBu_r',
                                       vmin=-vmax, vmax=vmax)
                self.figure.colorbar(contour, ax=ax, label='Electrostatic Potential (a.u.)')
                ax.set_xlabel(labels[0])
                ax.set_ylabel(labels[1])
            ...
            self.figure.colorbar(contour, ax=ax, label='Electrostatic Potential (synthetic)')
            ax.set_xlabel('x')
            ax.set_ylabel('y')
...
class MolecularOrbitalPopup(QDialog):
    ...
    def update_table(self):
        ...
        if homo and lumo:
            ...
            if gap > 0:
                ...
                properties.append(("Chemical Hardness", f"{gap/2:.4f} eV"))
                properties.append(("Chemical Potential", f"{-(homo.energy_ev + lumo.energy_ev)/2:.4f} eV"))
    ...
    def export_analysis(self):
        ...
        if filepath:
            tab_idx = min(self.tabs.currentIndex(), 5)
            canvases = [self.orbital_canvas, self.homo_canvas, self.lumo_canvas,
                       self.density_canvas, self.dipole_canvas, self.mep_canvas]
            canvases[tab_idx].figure.savefig(filepath, dpi=300, bbox_inches='tight')
