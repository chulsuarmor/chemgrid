# popup_molorbital.py (v1.0 - Molecular Orbital and Advanced Analysis)
"""
ChemDraw Pro: Molecular Orbital Visualization and Advanced Analysis
...
class OrbitalPlottingWidget(FigureCanvas):
    ...
    def plot_charge_distribution(self, X: np.ndarray, Y: np.ndarray, Z: np.ndarray):
        ...
        cbar = plt.colorbar(contour, ax=self.ax, label='Electron Density (a.u.)')
    ...
    def plot_molecular_potential(self, X: np.ndarray, Y: np.ndarray, Z: np.ndarray):
        ...
        cbar = plt.colorbar(contour, ax=self.ax, label='Electrostatic Potential (a.u.)')
