...
"""
ChemDraw Pro Phase D: Automated IUPAC Naming System
...
class StereochemistryAnalyzer:
    ...
    def assign_stereochemistry(mol: Chem.Mol) -> Dict[int, str]:
        ...
        try:
            ...
            # Extract R/S assignments
            Chem.rdMolTransforms.ComputeGasteigerCharges(mol)
...
class IUPACAnalyzerThread(QThread):
    ...
    def run(self):
        ...
        try:
            if not RDKIT_AVAILABLE:
                self.error.emit("[Phase D] RDKit not available")
                return
    ...
    def _build_rdkit_molecule(self) -> Optional[Chem.Mol]:
        """Build RDKit molecule from ChemDraw atoms and bonds"""
        try:
            mol = Chem.RWMol()
            atom_map = {}
