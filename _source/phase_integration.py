# [Phase Integration] phase_integration.py
"""
...
try:
    ...
    print("[Integration] Phase B not available")
...
try:
    ...
    print("[Integration] Phase C not available")
...
try:
    ...
    print("[Integration] Phase D not available")
...
class Phase3DPopupManager:
    ...
    def trigger_3d_popup_from_theory_layer(self, atoms: Dict, bonds: Dict, theory_data: Dict):
        ...
        if not PHASE_C_AVAILABLE:
            print("[Integration] Phase C (3D Popup) not available")
            return
        ...
        try:
            ...
            print(f"[Integration] 3D popup opened: {len(mol_data.atom_positions)} atoms")
            
        except Exception as e:
            print(f"[Integration Phase C Error] {e}")
...
class Phase2ESPCalculationManager:
    ...
    def start_esp_calculation(self, densities: List[ElectronicDensity], atom_positions: Dict):
        ...
        if not PHASE_B_AVAILABLE:
            print("[Integration] Phase B (ESP) not available")
            return
        ...
        try:
            ...
            print(f"[Integration] ESP calculation started: {len(densities)} density points")
            
        except Exception as e:
            print(f"[Integration Phase B Error] {e}")
    
    def _on_esp_progress(self, message: str):
        """Handle ESP progress message"""
        print(f"[Phase B Progress] {message}")
    
    def _on_esp_result(self, esp_map: Dict):
        """Handle ESP calculation result"""
        print(f"[Phase B Result] ESP map calculated: {len(esp_map)} points")
    ...
    def import_orca_densities(self, orca_result: OrcaCalculationResult) -> List[ElectronicDensity]:
        ...
        try:
            ...
            print(f"[Integration] Imported {len(densities)} electronic densities from ORCA")
            return densities
            
        except Exception as e:
            print(f"[Integration ORCA Import Error] {e}")
            return []
...
class Phase4IUPACAnalysisManager:
    ...
    def start_iupac_analysis(self, atoms: Dict, bonds: Dict):
        ...
        if not PHASE_D_AVAILABLE:
            print("[Integration] Phase D (IUPAC) not available")
            return
        ...
        try:
            ...
            print(f"[Integration] IUPAC analysis started")
            
        except Exception as e:
            print(f"[Integration Phase D Error] {e}")
    
    def _on_iupac_progress(self, message: str):
        """Handle IUPAC progress message"""
        print(f"[Phase D Progress] {message}")
    
    def _on_iupac_result(self, iupac_data):
        """Handle IUPAC analysis result"""
        self.iupac_data = iupac_data
        print(f"[Phase D Result] IUPAC: {iupac_data.iupac_name}")
        print(f"[Phase D Result] Stereochemistry: {iupac_data.stereo_descriptors}")
        print(f"[Phase D Result] Functional Groups: {iupac_data.functional_groups}")
...
class PhaseIntegrationManager:
    ...
    def on_molecule_updated(self, atoms: Dict, bonds: Dict, analysis_results: Dict = None):
        ...
        """
        print(f"[Integration] Molecule updated: {len(atoms)} atoms, {len(bonds)} bonds")
    ...
    def on_theory_layer_interaction(self, atoms: Dict, bonds: Dict, theory_data: Dict):
        ...
        """
        print("[Integration] Theory layer interaction detected")
    ...
    def on_orca_calculation_complete(self, orca_result: OrcaCalculationResult):
        ...
        """
        print("[Integration] ORCA calculation complete")
    ...
    def cleanup(self):
        ...
        print("[Integration] Starting cleanup process...")
        ...
        if self.esp_manager and self.esp_manager.esp_thread:
            if self.esp_manager.esp_thread.isRunning():
                print("[Integration] Stopping ESP calculation thread...")
                ...
                if not self.esp_manager.esp_thread.wait(5000):  # Wait max 5 seconds
                    print("[Integration] Warning: ESP thread did not finish cleanly")
        ...
        if self.iupac_manager and self.iupac_manager.iupac_thread:
            if self.iupac_manager.iupac_thread.isRunning():
                print("[Integration] Stopping IUPAC analysis thread...")
                ...
                if not self.iupac_manager.iupac_thread.wait(5000):  # Wait max 5 seconds
                    print("[Integration] Warning: IUPAC thread did not finish cleanly")
        
        # Close 3D popup
        if self.popup_manager:
            print("[Integration] Closing 3D popup...")
            self.popup_manager.close_popup()
        
        print("[Integration] Cleanup completed successfully")
...
def attach_phase_integration(canvas: QMainWindow) -> PhaseIntegrationManager:
    ...
    print("[Integration] Phase B-D integration attached to canvas")
    return manager
