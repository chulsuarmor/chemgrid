# spectrum_analyzer.py (v1.0 - IR/Raman Spectrum Analysis)
"""
...
try:
    ...
    print("[spectrum_analyzer.py] Matplotlib not available")
...
def parse_orca_frequencies(out_path: Path) -> SpectrumData:
    ...
    if not out_path.exists():
        print(f"[ERROR] ORCA output file not found: {out_path}")
    ...
    try:
        ...
        print(f"[SPECTRUM] Parsed {len(modes)} vibrational modes")
        print(f"[SPECTRUM] Frequency range: {min_freq:.1f} - {max_freq:.1f} cm^-1")
        print(f"[SPECTRUM] Max IR intensity: {max(ir_ints) if ir_ints else 0:.2f} km/mol")
...
def plot_raman_spectrum(
    ...
    # Plot peak positions
    if show_peaks:
        ax.scatter(spectrum_data.raman_frequencies, spectrum_data.raman_activities,
...
def plot_both_spectra(
    ...
    ax_raman.scatter(spectrum_data.raman_frequencies, spectrum_data.raman_activities,
...
if __name__ == "__main__":
    # Test spectrum parsing
    test_path = Path(r"C:\Users\김남헌\Desktop\organicdraw\test.out")
    if test_path.exists():
        spec_data = parse_orca_frequencies(test_path)
        print(f"Parsed {len(spec_data.modes)} vibrational modes")
        print(f"IR frequencies: {len(spec_data.ir_frequencies)}")
        print(f"Raman frequencies: {len(spec_data.raman_frequencies)}")
    else:
        print("[INFO] No test.out file found - module loaded successfully")
