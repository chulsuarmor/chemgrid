...
class ASKCOSClient:
    ...
    def expand_one(self, smiles: str, top_k: int = 10) -> List[ASKCOSPrecursor]:
        ...
        # [M849] retry with exponential backoff (Rule M: silent failure 금지)
        for attempt in range(self.MAX_RETRIES):
            for endpoint in endpoints_to_try:
                try:
                    ...
                    # v1/v2 모두 시도. raise_for_status는 4xx에서 발생.
                    if resp.status_code == 404 and endpoint == self.EXPAND_ONE_URL:
                        # v1 404 → v2 시도 (M645_W21 검증 시점 v1 endpoint 404 확인됨)
                        last_error = f"v1 HTTP 404 → v2 fallback"
                        logger.info("[ASKCOS] %s, trying %s", last_error, self.EXPAND_ONE_URL_V2)
                        continue
                    resp.raise_for_status()
                    data = resp.json()
                    break  # 성공
                except requests.exceptions.Timeout:
                    last_error = f"timeout at {endpoint} (attempt {attempt + 1}/{self.MAX_RETRIES})"
                    logger.warning("[ASKCOS] expand_one %s for: %s",
                                   last_error, smiles[:50])
                    continue
                except requests.exceptions.ConnectionError:
                    last_error = f"Connection error (attempt {attempt + 1}/{self.MAX_RETRIES})"
                    logger.warning("[ASKCOS] %s — server unreachable", last_error)
                    break  # 양쪽 모두 connection 실패면 break
                except requests.exceptions.HTTPError as e:
                    last_error = f"HTTP error at {endpoint}: {e}"
                    logger.warning("[ASKCOS] %s", last_error)
                    continue
                except Exception as e:
                    last_error = f"Unexpected error at {endpoint}: {e}"
                    logger.warning("[ASKCOS] %s", last_error)
                    continue
            if data is not None:
            ...
            if attempt < self.MAX_RETRIES - 1:
                ...
                logger.info("[ASKCOS] expand_one retry %d/%d after %.1fs",
                            attempt + 1, self.MAX_RETRIES, _wait)
                time.sleep(_wait)

        if data is None:
            # Rule M: silent failure 차단 — 마지막 에러 로깅 + 인스턴스 추적
            self._last_error = last_error or "unknown"
            logger.warning("[ASKCOS] expand_one: all endpoints/retries failed (last: %s)",
                           self._last_error)
            return []
