...
w.deleteLater()
if ok:
    print('PASS MainWindow visible', flush=True)
else:
    print('FAIL MainWindow not visible', flush=True)
# M1355: os._exit bypasses non-daemon background threads spawned by MainWindow.__init__
# (network clients, schedulers) that prevent normal Python subprocess exit.
# flush() before _exit ensures PASS/FAIL line is captured by subprocess.run stdout.
sys.stdout.flush()
os._exit(0 if ok else 1)
""", timeout_sec=30.0))  # M1355: was 120s; os._exit makes subprocess exit in ~10s (M428-cycle35 stale)
