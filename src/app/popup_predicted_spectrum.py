...
class PredictedSpectrumPopup(QDialog):
    ...
    def _build_ui(self):
        ...
        tabs = QTabWidget()
        tabs.addTab(FigureCanvas(_make_ir_figure(spec.ir_peaks)), "IR")
        # [NEW v3] Raman에 IR peaks 전달 → ghost layer
        tabs.addTab(FigureCanvas(_make_raman_figure(spec.raman_peaks, spec.ir_peaks)), "Raman")
        tabs.addTab(FigureCanvas(_make_nmr_h1_figure(spec.h1_nmr_peaks, spec.formula, self.smiles)), "¹H-NMR")
        tabs.addTab(FigureCanvas(_make_nmr_c13_figure(spec.c13_peaks, spec.formula, self.smiles)), "¹³C-NMR")
        tabs.addTab(FigureCanvas(_make_uvvis_figure(spec.uvvis_peaks)), "UV-Vis")
