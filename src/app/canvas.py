...
class MoleculeCanvas(QWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        if (
            ...
            if self.view_state in ["Lewis", "Theory"]:
                ...
                for k in self.atoms:
                    # [ISSUE-3 FIX] theory_map은 round(x,2) 키로 저장됨
                    # 원본 k 키로 조회하면 항상 miss → 원본 좌표로 fallback → 선택 좌표 불일치
            ...
            if self.view_state == "Theory":
                ...
                if is_drag_select:
                    ...
                    if self.selected_atoms:
                        self.selected_molecule_keys = set(self.selected_atoms)
                        self._compute_molecule_bbox()
                        self._fetch_molecule_name()
                        self.molecule_selected.emit(True)
                        print(f"[3D-FIX] 드래그 선택 완료: {len(self.selected_atoms)}개 원자, btn_3d 활성화")
                    else:
                        # 빈 영역 드래그 → 선택 해제
                        if self.selected_molecule_keys:
                            self._deselect_molecule()
    ...
    def get_smiles(self):
        ...
        try:
            ...
            for coord_key, atom_data in self.atoms.items():
                ...
                # 원소 기호가 없고 결합도 없으면 무시 (단순 마커/전하 도구 클릭 방지)
                if not element and degrees.get(coord_key, 0) == 0:
                    continue
    ...
    def _find_atom_at_theory(self, l_pos):
        ...
        [FIX 3D-3] analysis_results 없어도 원본 좌표로 fallback (Theory 전환 직후 클릭 실패 방지)
        Returns: atom_key tuple 또는 None"""
        # [FIX 3D-3] analysis_results가 None이어도 원본 좌표로 탐색 (early return 제거)
        t_map = {}
        if self.analysis_results:
        ...
        for k in self.atoms:
            pt = t_map.get(k, QPointF(*k))  # map 없으면 원본 좌표 사용
            px, py = pt.x(), pt.y()
    ...
    def draw_atom_group(self, p, pt, data, sel):
        ...
        for d, sym in data["attach"].items():
            ...
            if sym == "..":
                ...
                p.save()
                p.setPen(QPen(Qt.GlobalColor.black, 3.0))
                p.drawLine(QPointF(sx - 10, sy), QPointF(sx + 10, sy))
                p.drawLine(QPointF(sx, sy - 10), QPointF(sx, sy + 10))
                p.restore()
            elif sym == "-":
                p.save()
        ...
        # [v4 명령1 Fix v3] charge 필드 렌더링 — 원소 기호 옆 위첨자, 항상 검은색
        # 탄소(main="")이면 원자 위치 우상단에 고정 오프셋으로 표시
        ...
        if charge_sym:
            ...
            # [BUG-FIX] +/- 기호를 원자 우상단 위첨자로 표시
            # 수정 전: label_w=6(default) → cx≈pt.x()+5, cy≈pt.y()-3 → 본드 교차점 겹침
            # 수정 후: cx=pt.x()+label_w/2+8, cy=pt.y()-fm.height()/2 → 확실한 위첨자 위치
            label_w = fm.horizontalAdvance(main) if main else 0
            cx_charge = pt.x() + label_w / 2 + 8   # 우측 오프셋 8px
            cy_charge = pt.y() - fm.height() / 2     # 원자 중심 절반 위: 위첨자 위치
            p.setPen(Qt.GlobalColor.black)
            p.drawText(QPointF(cx_charge, cy_charge), charge_sym)
            p.restore()
