# orca_interface.py (v3.10 - %plots cube verified + xtb GFN2-xTB integration)
"""
...
# ============================================================================
# XTB GFN2-xTB INTEGRATION (DFT-002)
# ============================================================================
...
def find_xtb_executable() -> Optional[Path]:
    ...
    Search order:
      1. _SCRIPT_DIR / "xtb" / "xtb.exe"  (or "xtb" on Linux/Mac)
      2. _SCRIPT_DIR.parent / "xtb" / "xtb.exe"
      3. _SCRIPT_DIR.parent.parent / "xtb" / "xtb.exe"
      4. System PATH (shutil.which("xtb"))

    Returns:
        Path to xtb executable, or None if not found.
    """
...
def run_xtb_charges(
    ...
    if result.returncode != 0:
        logger.warning("xtb exited with code %d: %s", result.returncode, result.stderr[:500])
        return {}
...
if __name__ == "__main__":
    ...
    print("[orca_interface.py v3.10] Portable ORCA DFT Interface + xtb GFN2-xTB")
    print(f"  Script dir: {_SCRIPT_DIR}")
    print(f"  ORCA found: {find_orca_executable()}")
    print(f"  xtb found:  {find_xtb_executable()}")
    validate_orca_installation()
    validate_xtb_installation()
    print("[orca_interface.py v3.10] Ready")
