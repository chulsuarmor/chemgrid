...
class LewisRenderer:
    ...
    def _draw_lone_pair(painter, center, direction, gap=22):
        ...
        # [v8.4] gap 최소값 보장 — 원자 기호와 겹치지 않도록
        effective_gap = max(gap, 18)
        pos = center + direction * effective_gap

        # 수직 방향 벡터 (두 점을 좌우로 배치) — 간격 4.0px로 확대
        perp = QPointF(-direction.y(), direction.x()) * 4.0

        # [v8.4] 점 크기 3.0으로 확대 + 테두리 추가 (시인성 향상)
        painter.setBrush(Qt.GlobalColor.black)
        painter.setPen(QPen(Qt.GlobalColor.black, 0.5))
        painter.drawEllipse(pos + perp, 3.0, 3.0)
        painter.drawEllipse(pos - perp, 3.0, 3.0)
...
class TheoryRenderer:
    ...
    def get_bond_gap(pt_key, atoms_data):
        ...
        if symbol and symbol.strip() and symbol != "C":
            ...
            user_placed_h = sum(1 for sym in atom.get("attach", {}).values() if sym == "H")
    ...
    def render(painter, atoms, bonds, analysis, selected_atoms=None, selected_bonds=None):
        ...
        for pt_key, atom_data in analysis.get("atoms", {}).items():
            ...
            user_placed_h = sum(1 for sym in atom_data.get("attach", {}).values() if sym == "H")
            implicit_h = max(0, atom_data.get("h_count", 0) - user_placed_h)

            # 표시 라벨: "O" + "H" + (숫자 subscript if > 1)
            display_label = symbol
            if implicit_h == 1:
                display_label = symbol + "H"
            elif implicit_h > 1:
        ...
        for pt_key, atom_data in analysis.get("atoms", {}).items():
            ...
            if lp_count > 0:
                ...
                # 빈 각도 영역 찾아 lp_count개 비공유전자쌍 균등 배치
                # (O=2쌍, Cl=3쌍, N=1쌍 등 정확히 표시)
                n_bonds = len(occupied_angles)
                lp_angles = []
                if n_bonds == 0:
                    # 결합 없음: 위쪽부터 균등 배치
                    for i in range(lp_count):
                        lp_angles.append(270 + i * (360 / max(lp_count, 1)))
                elif n_bonds == 1:
                    # 결합 1개: 반대편을 중심으로 부채꼴 배치
                    opp = (occupied_angles[0] + 180) % 360
                    spread = 50  # 부채꼴 각도
                    if lp_count == 1:
                        lp_angles = [opp]
                    elif lp_count == 2:
                        lp_angles = [opp - spread/2, opp + spread/2]
                    else:
                        for i in range(lp_count):
                            lp_angles.append(opp - spread + i * spread * 2 / max(lp_count - 1, 1))
                else:
                    # 결합 2+개: 결합 사이 빈 각도 중 가장 넓은 곳에 배치
                    sorted_a = sorted(occupied_angles)
                    gaps = []
                    for i in range(len(sorted_a)):
                        ...
                        if gap == 0:
                            gap = 360
                        gaps.append((gap, a1))
                    gaps.sort(key=lambda g: -g[0])  # 가장 넓은 갭 먼저
                    placed = 0
                    for gap_size, gap_start in gaps:
                        if placed >= lp_count:
                            break
                        # 이 갭에 배치할 LP 수
                        to_place = min(lp_count - placed, max(1, int(gap_size / 90)))
                        for j in range(to_place):
                            offset = gap_size * (j + 1) / (to_place + 1)
                            lp_angles.append((gap_start + offset) % 360)
                            placed += 1

                # 각 비공유전자쌍 그리기 [v8.4] 크기/간격 확대
                painter.setBrush(Qt.GlobalColor.black)
                painter.setPen(QPen(Qt.GlobalColor.black, 0.5))
                lp_dist = 22
                for lp_ang in lp_angles:
                    rad = math.radians(lp_ang)
                    lp_pos = QPointF(center.x() + math.cos(rad) * lp_dist,
                                     center.y() + math.sin(rad) * lp_dist)
                    perp = QPointF(-math.sin(rad), math.cos(rad)) * 4.0
                    painter.drawEllipse(lp_pos + perp, 3.0, 3.0)
                    painter.drawEllipse(lp_pos - perp, 3.0, 3.0)
