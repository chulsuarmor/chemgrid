# popup_spectrum.py (v1.1 - IR/Raman Spectrum Viewer Popup)
"""
...
import sys
import os
# Add parent directory to path to import spectrum_pdf_exporter
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
try:
    from spectrum_pdf_exporter import (
        SpectrumPDFExporter, SpectrumMetadata, 
        SpectrumData as PDFSpectrumData, SpectrumPeakData
    )
    PDF_EXPORT_AVAILABLE = True
except ImportError:
    PDF_EXPORT_AVAILABLE = False
    logger.warning("spectrum_pdf_exporter not available")
...
class SpectrumPopup(QDialog):
    ...
    def init_ui(self):
        ...
        export_btn = QPushButton("Export Image/CSV...")
        ...
        pdf_btn.setEnabled(PDF_EXPORT_AVAILABLE)
    ...
    def export_pdf_report(self):
        ...
        if not PDF_EXPORT_AVAILABLE:
            QMessageBox.warning(self, "Error", "PDF Export module not available")
            return
        ...
        try:
            ...
            metadata = SpectrumMetadata(
                molecule_name="Analyzed Molecule",  # TODO: Get actual name
                molecular_formula="Unknown",
                calculation_method="DFT B3LYP/6-31G(d)",
                software="ORCA 6.1.1",
                final_energy=-1000.0,
                convergence_status="Converged" if self.spectrum_data.converged else "Failed"
            )
            
            exporter = SpectrumPDFExporter(metadata)
            
            # 2. Generate and Add IR Spectrum
            ir_peaks = []
            for mode in self.spectrum_data.modes:
                if mode.intensity > 10.0:  # Filter weak peaks
                    ir_peaks.append(SpectrumPeakData(
                        frequency=mode.frequency,
                        intensity=mode.intensity,
                        label=str(mode.frequency),
                        assignment="vibration",
                        unit="cm⁻¹"
                    ))
            
            # Generate IR Image
            ir_fig = plot_ir_spectrum(self.spectrum_data, title="IR Spectrum")
            ir_img_path = str(Path(file_path).parent / "temp_ir_spectrum.png")
            ir_fig.savefig(ir_img_path, dpi=300, bbox_inches='tight')
            
            ir_data = PDFSpectrumData(
                spectrum_type="IR",
                peaks=ir_peaks,
            ...
            # 3. Generate and Add Raman Spectrum
            raman_peaks = []
            for mode in self.spectrum_data.modes:
                if mode.raman_activity > 1.0:  # Filter weak peaks
                    raman_peaks.append(SpectrumPeakData(
                        frequency=mode.frequency,
                        intensity=mode.raman_activity,
                        label=str(mode.frequency),
                        assignment="vibration",
                        unit="cm⁻¹"
                    ))
            
            # Generate Raman Image
            raman_fig = plot_raman_spectrum(self.spectrum_data, title="Raman Spectrum")
            raman_img_path = str(Path(file_path).parent / "temp_raman_spectrum.png")
            raman_fig.savefig(raman_img_path, dpi=300, bbox_inches='tight')
            
            raman_data = PDFSpectrumData(
                spectrum_type="Raman",
                peaks=raman_peaks,
                image_path=raman_img_path
            )
            exporter.add_spectrum("Raman", raman_data)
            ...
            # Cleanup temp images
            try:
                if os.path.exists(ir_img_path): os.remove(ir_img_path)
                if os.path.exists(raman_img_path): os.remove(raman_img_path)
            except:
                pass
...
def launch_spectrum_viewer(orca_output_path: str, parent=None):
    ...
    """
    try:
        path = Path(orca_output_path)
        if path.exists():
            spectrum_data = parse_orca_frequencies(path)
        else:
            # Fallback for testing/Benzene demo
            from PyQt6.QtWidgets import QMessageBox
            reply = QMessageBox.question(
                parent, 
                "File Not Found", 
                f"ORCA output not found:\n{orca_output_path}\n\nLoad Benzene Mock Data instead?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            
            if reply == QMessageBox.StandardButton.Yes:
                spectrum_data = create_benzene_mock_data()
            else:
                return None
        
        if len(spectrum_data.modes) == 0:
            # If parsing failed but file exists, try mock data
            from PyQt6.QtWidgets import QMessageBox
            reply = QMessageBox.question(
                parent,
                "No Data",
