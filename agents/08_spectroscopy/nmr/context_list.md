# 📋 🟢 NMR 분광학 — Task List
## 최종 업데이트: 2026-02-28 23:56 / 중간관리자(08) 직접 수행

- [x] S3: `popup_nmr.py` matplotlib `backend_qt5agg` → `backend_qtagg`
- [x] S7: `popup_nmr.py` `setSizePolicy(4, 4)` → `QSizePolicy.Policy.Expanding` enum 사용
- [x] S5: `popup_nmr.py` `print()` → `logging` 교체 (1건)
- [ ] BaseSpectrumPopup 상속 적용 → Phase 7에서 Agent 06 통합 시 수행

### 수행자
- 중간관리자(08)가 하위 에이전트 미착수로 인해 직접 패치 수행 (2026-02-28 23:55)
- AST 검증 완료 (conda chemgrid Python 3.12.12)
