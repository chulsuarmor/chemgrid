# 📋 Agent 08c: UV-Vis — Context Plan
## 최종 업데이트: 2026-02-28 22:36 / 중간관리자(08) 지시

### 1. 역할
- UV-Vis 분광학 모듈 리팩토링 담당
- 담당 파일: `popup_uvvis.py`
- 상위: Agent 08 (분광학 중간관리자)

### 2. 현재 상태
- **파일이 `_source/` 원본과 100% 동일 — 수정 0%**
- 이슈 2건 할당 (S3, S5) + BaseSpectrumPopup 적용
...
| 2 | 🟡 | S5 | `print()` → `import logging; logger = logging.getLogger(__name__); logger.info/debug()` 교체 |
| 3 | 🟢 | 리팩토링 | `UVVisPopup(QDialog)` → `UVVisPopup(BaseSpectrumPopup)` 상속 적용. `_parse_data()` (TDDFTParser 연동), `_setup_ui()`, `get_spectrum_data_for_pdf()` 구현. (base_spectrum.py는 중간관리자가 생성 예정) |

### 4. 공통 규칙 (MANDATORY)
- 절대 경로 금지 → `__file__` 기반 상대 경로 필수
- Python: `py` 명령어 사용
- 자가 검증: `py -c "import ast; ast.parse(open('popup_uvvis.py', encoding='utf-8').read())"`
- 원본 `_source/` 절대 수정 금지

### 5. 마일스톤
- Phase 1: 코드 분석 (중간관리자 완료)
- Phase 2: S3(backend) + S5(logging) 수정
- Phase 3: BaseSpectrumPopup 적용
- Phase 4: 자가 검증 → 중간관리자에게 보고
...
> 
> ### 작업 순서
> 1. S3 (backend_qtagg) → S5 (print→logging) 수정
> 2. BaseSpectrumPopup 상속 적용
> 3. 자가 검증 후 중간관리자에게 보고
