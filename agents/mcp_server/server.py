...
from google import generativelanguage as genai
...
app = FastAPI(title="Gemini MCP Server & ChemGrid Tools")
...
COMPLEXITY_LEVELS = {
    ...
    4: {
        ...
        "actions": [
            {"tool": "select", "category": "bond", "value": "single"},
            {"tool": "select", "category": "element", "value": "C"},
            # 5-membered ring (Corrected 5 points)
            ...
            {"tool": "drag", "start_x": 6, "start_y": 6, "end_x": 6, "end_y": 5},
            {"tool": "drag", "start_x": 6, "start_y": 5, "end_x": 5, "end_y": 4},
            
            # Double bonds: p2-p3 (4,5->5,6), p4-p5 (6,6->6,5)
            # Use drag again to toggle bond order safely
...
async def analyze_image(
    ...
    try:
        ...
        if file.filename.endswith('.pdf'):
            ...
            image = images[0].convert('RGB')  # 첫 페이지 RGB 변환
        else:
...
if __name__ == "__main__":
    print(f"Starting Gemini MCP Server & Tooling...")
