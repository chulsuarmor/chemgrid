...
class LewisRenderer:
    ...
    def _render_atom_symbols(painter, analysis, t_map, selected_atoms):
        ...
        for pt_key, atom_data in analysis["atoms"].items():
            ...
            atom_color = (Qt.GlobalColor.blue if is_selected
    ...
    def _render_formal_charges(painter, analysis, t_map):
        """
        형식전하(+/-) 렌더링 — 원자 기호 우상단에 ChemDraw 스타일로 표시.
        예: N⁺, O⁻
        """
        atoms_data = analysis.get("atoms", {})
        for pt_key, atom_data in atoms_data.items():
            ...
            # 전하 기호 결정
            if formal_charge > 0:
                charge_text = "+" * abs(formal_charge)
                charge_color = QColor(220, 50, 50)  # 빨간색 (양전하)
            else:
                charge_text = "−" * abs(formal_charge)  # U+2212 (마이너스 기호)
                charge_color = QColor(50, 50, 220)  # 파란색 (음전하)
