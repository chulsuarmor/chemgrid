# orca_interface.py (v2.03 - PORTABLE PATH FIX + CRITICAL FIX: Strict Column Check)
"""
...
PORTABLE FIX v2.03:
✅ C1 FIX: ORCA 경로를 __file__ 기반 상대 경로로 변경
   - 하드코딩된 절대 경로 제거
   - zip 배포 시 어떤 디렉토리에서도 작동
"""
...
# ============================================================================
# CONFIGURATION — C1 FIX: 포터블 경로 시스템
# ============================================================================

# ✅ C1 FIX: __file__ 기반 상대 경로 (하드코딩 절대 경로 제거)
# ❌ 이전: ORCA_PATH = Path(r"C:\Users\김남헌\Desktop\organicdraw\Orca.6.1.1")
_SCRIPT_DIR = Path(os.path.dirname(os.path.abspath(__file__)))
ORCA_PATH = _SCRIPT_DIR / "Orca.6.1.1"
ORCA_EXE = ORCA_PATH / "orca.exe"
...
def create_calculation_workflow(
    ...
    """
    if work_dir is None:
        # ✅ C5 FIX: 포터블 경로 — 스크립트 위치 기반
        work_dir = _SCRIPT_DIR / "orca_calcs"
...
if __name__ == "__main__":
    ...
    print("[orca_interface.py v2.03] Initializing Phase A module...")
    print(f"[orca_interface.py v2.03] ORCA_PATH = {ORCA_PATH}")
    print(f"[orca_interface.py v2.03] ORCA_EXE  = {ORCA_EXE}")
    validate_orca_installation()
    print("[orca_interface.py v2.03] Ready with STRICT COLUMN CHECK + PORTABLE PATHS")
