...
"""
Centralized coordinate handling for ChemGrid
...
# ============================================================================
# RING DETECTION — 고리 내 이중결합 방향 보정용 (U5)
# ============================================================================

def find_shortest_ring_through_bond(
    k1: Tuple[float, float],
    k2: Tuple[float, float],
    bonds: Dict,
) -> list:
    """
    결합 (k1, k2)를 포함하는 최소 고리를 BFS로 탐색.
    
    Args:
        k1: 결합 시작점 좌표 키 (x, y)
        k2: 결합 끝점 좌표 키 (x, y)
        bonds: 전체 결합 딕셔너리 {(ka, kb): order_or_data, ...}
    
    Returns:
        고리를 구성하는 좌표 키 리스트 [k1, ..., k2] 또는 고리가 없으면 None
    """
    ...
    for bond_key in bonds:
        a, b = bond_key[0], bond_key[1]
        adj.setdefault(a, set()).add(b)
        adj.setdefault(b, set()).add(a)

    if k1 not in adj or k2 not in adj:
        return None

    # BFS: k1에서 출발, 직접 간선 (k1→k2)을 사용하지 않고 k2에 도달하는 최단 경로
    visited = {k1}
    queue = deque()
    ...
    for neighbor in adj.get(k1, set()):
        if neighbor != k2:
            ...
            queue.append((neighbor, [k1, neighbor]))

    while queue:
        current, path = queue.popleft()
        if len(path) > 9:  # 화학적으로 의미 있는 고리 크기 제한 (최대 8원환)
            continue
...
def get_ring_center_for_bond(
    k1: Tuple[float, float],
    k2: Tuple[float, float],
    bonds: Dict,
) -> 'QPointF | None':
    """
    결합 (k1, k2)가 포함된 고리의 중심점을 반환.
    ...
    Returns:
        고리 중심 QPointF 또는 고리가 없으면 None
    """
    ring = find_shortest_ring_through_bond(k1, k2, bonds)
    if ring is None:
        return None

    cx = round(sum(pt[0] for pt in ring) / len(ring), 2)
    cy = round(sum(pt[1] for pt in ring) / len(ring), 2)
