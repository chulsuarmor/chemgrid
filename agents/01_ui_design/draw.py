...
# 2. 분석 엔진을 캔버스에 장착합니다 (여기에 추가)
    ...
    def _analyze_dft_electron_density(self, orca_result):
        ...
        try:
            ...
            # [C3 수정] self가 곧 canvas이므로 self.update() 사용
            self.update()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.tb.setIconSize(QSize(34, 34))
        self.tb.setMinimumHeight(58)
        self.addToolBar(self.tb)

        logo_action = QAction(load_icon("logo.png"), "ChemGrid", self)
        self.tb.addAction(logo_action)
        self.tb.addSeparator()

        file_menu = QMenu("파일", self)
        file_menu.addAction("저장 (.chem)", self.save_file)
        file_menu.addAction("불러오기 (.chem)", self.load_file)
        file_btn = QAction("파일", self); file_btn.setMenu(file_menu)
        self.tb.addAction(file_btn)
        ...
        export_btn = QAction("내보내기", self); export_btn.setMenu(export_menu)
        self.tb.addAction(export_btn)
        ...
        # ==========================================
        # [명령 1] 2단 툴바 (tb2): 그리기 도구
        # ==========================================
        self.tb2 = QToolBar("그리기")
        self.tb2.setMovable(False)
        self.tb2.setIconSize(QSize(34, 34))
        self.tb2.setMinimumHeight(58)
        ...
        for n in ["Bond", "Pen"]:
            ...
            a.triggered.connect(self.create_handler(n)); self.tb2.addAction(a); grp.addAction(a)
        ...
        self.arrow_action.triggered.connect(self.create_handler("Arrow"))
        self.tb2.addAction(self.arrow_action); grp.addAction(self.arrow_action)

        # Eraser
        eraser_icon = load_icon("eraser.png", mode_name="Eraser")
        eraser_action = QAction(eraser_icon, "Eraser", self); eraser_action.setCheckable(True)
        eraser_action.triggered.connect(self.create_handler("Eraser"))
        self.tb2.addAction(eraser_action); grp.addAction(eraser_action)

        # [명령 4] 텍스트 도구 (신규)
        self.text_action = QAction("T", self)
        self.text_action.setToolTip("텍스트 상자")
        self.text_action.setCheckable(True)
        self.text_action.triggered.connect(self.create_handler("Text"))
        self.tb2.addAction(self.text_action); grp.addAction(self.text_action)
        ...
        # 원소 및 화학 기호 도구
        for n in ["H", "R", "LonePair", "Radical", "Positive", "Negative", "O", "N", "P", "S", "F", "Cl", "Br", "I"]:
            img_info = tool_icons.get(n, ("", n if len(n) <= 2 else None))
            img_path, sym = img_info
            icon = load_icon(img_path, mode_name=n, symbol_text=sym)
            a = QAction(icon, n, self); a.setCheckable(True)
