...
## GIT-PUSH-LFS-001: git push HTTP 500 회피 패턴 (M1384, 2026-05-18)

### 증상
- `git push origin <branch>` 시 항상 HTTP 500 RPC failed
- `send-pack: unexpected disconnect` + `abort upload after having sent N bytes`
- `git config http.postBuffer 524288000` 등 설정 후에도 동일

### 원인 진단
```bash
git ls-tree -r --long HEAD | sort -k4 -rn | head -10  # 대용량 blob 확인
git count-objects -v  # size-pack 확인 (16GB+ = 문제)
```
- git history에 2GB+ 바이너리(exe/zip/pkg 등) 포함 시 GitHub HTTPS 2GB 한도 초과
- 어떤 http 설정도 서버 측 한도를 우회 불가

### 단기 회피: GitHub Contents API 파일별 업로드
```python
import base64, urllib.request, json
TOKEN = "..."  # gh auth token
def upload_file(repo, filepath, content_bytes, sha=None, branch="master"):
    b64 = base64.b64encode(content_bytes).decode()
    data = {"message": f"fix: upload {filepath}", "content": b64, "branch": branch}
    if sha:
        data["sha"] = sha
    # GET /repos/{repo}/contents/{path} 로 sha 먼저 조회
    # PUT /repos/{repo}/contents/{path} 로 업로드
```
- 10MB 이하 파일만 가능 (GitHub API 단일 파일 한도)
- 코드 파일 동기화에 적합

### 근본 해결 (CT 결정 필요)
1. `pip install git-filter-repo`
2. `git filter-repo --strip-blobs-bigger-than 50M`
3. `git push --force origin master` (force push — 사용자 확인 필수)
4. SSH 키 설정: `ssh-keygen -t ed25519 -C "email"` → GitHub SSH 등록 → `git remote set-url origin git@github.com:...`
